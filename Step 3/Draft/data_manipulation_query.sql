...
--find_restaurant.html
	...
	--display selected restaurant information from list of matching restaurants
		SELECT r.name, c.type, l.street_name, l.city, l.state, l.zip, r.rating
		FROM restaurants r 
	...
	--display reviews when see reviews button is clicked
		SELECT SUM(review) AS number_of_likes
		FROM reviews_restaurant_customer
		WHERE review = 1;

		SELECT SUM(review) AS number_of_dislikes
		FROM reviews_restaurant_customer
		WHERE review = 0;
