...
--remove_review.html
	...
	--display all current reviews by a customer
...
--delete.php
	...
	--update the rating for restaurant with removed review
		UPDATE `restaurants` 
        SET `rating` = (
        	SELECT AVG (review) 
        	FROM `reviews_restaurants_customers` 
...
--find_restaurant.html
	...
	--get all restaurants with matching name, zip code, city, or cuisine type
        ...
        INNER JOIN location ON location.id = restaurants.locationID
        INNER JOIN cuisine ON cuisine.id = restaurants.cuisineID
        WHERE restaurants.name LIKE %$name% OR location.zip = $zip OR location.city = $city OR restaurants.cuisineID = $cuisineID;
